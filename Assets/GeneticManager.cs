...
{
    [Header("References")]
    public Submarine controllerSubmarine;
    ...
    {
        controllerSubmarine.ResetWithNetwork(population[currentGenome]);
    }
    ...
    private void FillPopulationWithRandomValues(NNet[] newPopulation, int startingIndex)
    {
        while (startingIndex < initialPopulation)
        {
            ...
            newPopulation[startingIndex].Initialise(controllerSubmarine.LAYERS, controllerSubmarine.NEURONS);
            startingIndex++;
        }
    }
    ...
    private void Crossover(NNet[] newPopulation)
    {
        for (int i = 0; i < numberToCrossover; i += 2)
        {
            ...
            Child1.Initialise(controllerSubmarine.LAYERS, controllerSubmarine.NEURONS);
            Child2.Initialise(controllerSubmarine.LAYERS, controllerSubmarine.NEURONS);
    ...
    private NNet[] PickBestPopulation()
    {
        ...
        {
            newPopulation[naturallySelected] = population[i].InitialiseCopy(controllerSubmarine.LAYERS, controllerSubmarine.NEURONS);
